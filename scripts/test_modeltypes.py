...
xtopics2 = d_topics + ['d_%s'%d_topic for d_topic in d_topics] + ['performance2']
ytopic = 'safety2'
...
print("Now with performance2 as well in xtopics")
for config in configs:
    print('Training for %s'%config)
    # Loop train models
    for modelname in models:
        gkf = GroupKFold(n_splits=int(groups2[config][-1])+1)

        idm = 0   
        best_score = -10  
        for train, test in gkf.split(X2[config], y2[config], groups=groups2[config]):
            # Fit model
            m[modelname].fit(X2[config][train],y2[config][train])
            # Score 
            r2score = m[modelname].score(X2[config][test],y2[config][test])
            r2score_own = m[modelname].score(X2[config][train],y2[config][train])
            
            # print('$%s_model= %s, idx=%s, own score = %s, average score = %s'%(ytopic,modelname,idx,round(r2score[modelname][idx][idx],4),round(np.average(r2score[modelname][idx]),4)))
            # Save model
            if r2score > best_score:
                best_filename = dir_models + "%s_%s_%s_2best.pkl"%(ytopic,modelname,config)
                model_best = m[modelname]
                best_score = r2score
