...
idx = 0
for config in configs:
    topics = [[df[config][idx]['safety2'],df[config][idx]['performance2_2']],[df[config][idx]['narrowness1'],df[config][idx]['density1']]]
...
m = dict()
# models = ['knn','SVR_rbf','SVR_poly','RF']
models = ['knn','SVR_rbf']
# scoring_metric = 'neg_mean_squared_error'
scoring_metric = 'r2'


m['knn'] = GridSearchCV(KNeighborsRegressor(),
        ...
        for train, test in gkf.split(X[config], y[config], groups=groups[config]):
            ...
            if idm == 0:
                ...
                for x in m[modelname].best_params_:
                    parnames[i] = x
                    i += 1
                ws.append(['Model name','Trainset number'] + parnames + ['test score','own score'])
            # Parameter values
            pars = ['','','','']
            i = 0
            for x in m[modelname].best_params_:
                ...
                i += 1
            ws.append(["%s_%s_%s_%s.pkl"%(ytopic,modelname,config,idm), idm] + pars + [round(r2score,5),round(r2score_own,5)])
